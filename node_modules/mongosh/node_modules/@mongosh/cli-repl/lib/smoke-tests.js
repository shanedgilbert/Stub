...
async function runSmokeTest(executable, args, input, output) {
    ...
    try {
        ...
        console.error({ status: 'success', input, output, stdout, executable, args: args.map(history_1.redactURICredentials) });
    }
    catch (err) {
        console.error({ status: 'failure', input, output, stdout, executable, args: args.map(history_1.redactURICredentials) });
        throw err;
    }
}
